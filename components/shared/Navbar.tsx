...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full lg:px-64 md:px-24 sm:px-16 pt-5 px-10 transition-transform duration-500 transform  ${
        visible ? "translate-y-0" : "-translate-y-full"
      } z-10`}
    >
      <div className="dark:bg-dark-600 bg-gray-100 rounded-2xl  flex px-5 justify-between duration-1000 items-center shadow-lg shadow-black/25 dark:shadow-black/70">
        <Link href="/" className="max-md:p-2">
          <h1 className="font-conthrax lg:text-xl py-3">ALAMSYAH.ID</h1>
        </Link>
            ...
            <div className="flex gap-12">
              {navButtons.map((nav, i) => (
                ...
                >
                  {nav.name}
                  <div
                    className={`${nav.LineColor} rounded-full w-full h-1   ${
                      path === nav.url ? "flex" : "hidden group-hover:flex"
                    }`}
                  />
                </Link>
              ))}
